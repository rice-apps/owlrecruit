...
export default function ClubStatusCard({ userId }: ClubStatusCardProps) {
    ...
    async function fetchClubData() {
      try {
        ...
        if (adminOrgsResult.error) {
          throw new Error(`Failed to fetch admin orgs: ${adminOrgsResult.error.message}`);
        }
  ...
  const hasNoData = clubData.adminOrgs.length === 0 && clubData.applications.length === 0;
  ...
  if (hasNoData) {
    return (
      <StatusCard>
        <div className="text-center py-8">
          <p className="text-muted-foreground">No admin organizations or applications found.</p>
        </div>
      </StatusCard>
    );
  }
              ...
              {clubData.adminOrgs.map((adminOrg) => (
                ...
                >
                  <div className="flex-1">
                    <p className="font-medium">{adminOrg.org_name || 'Unknown Club'}</p>
                  </div>
