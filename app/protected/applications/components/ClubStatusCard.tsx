...
export default function ClubStatusCard({ userId }: ClubStatusCardProps) {
  ...
  const handleMembershipClick = (membership: ClubMembership) => {
    if (isUserAdmin(membership.org_id)) {
      router.push(`/protected/applications/clubs/admin/${membership.org_id}`);
    } else {
      router.push(`/protected/applications/clubs/${membership.org_id}`);
    }
  };
    ...
    async function fetchClubData() {
      try {
        ...
        const [orgsResult, rolesResult] = await Promise.all([
          orgIds.size > 0 ? supabase
            .from('orgs')
            .select('id, name')
            .in('id', Array.from(orgIds)) : { data: [], error: null },
          roleIds.size > 0 ? supabase
            .from('roles')
            .select('id, name')
            .in('id', Array.from(roleIds)) : { data: [], error: null }
        ]);
        ...
        const roleMap = new Map(rolesResult.data?.map(role => [Number(role.id), role.name]) || []);
  ...
  const hasNoData = clubData.memberships.length === 0 && clubData.applications.length === 0;
  ...
  if (hasNoData) {
    return (
      <StatusCard>
        <div className="text-center py-8">
          <p className="text-muted-foreground">No club memberships or applications found.</p>
        </div>
      </StatusCard>
    );
  }
